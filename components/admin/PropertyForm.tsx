...
export default function PropertyForm({ initial }: { initial?: AdminProperty }) {
  ...
  return <form className="property-form" onSubmit={submit} noValidate>
    ...
    <section className="form-section"><div className="form-section-head"><span>06</span><div><h2>Video de la propiedad</h2><p>Sube un video MP4 o MOV.</p></div></div>
      <label className="video-picker"><span><strong>Seleccionar video MP4 o MOV</strong><small>Puedes subir videos directamente desde iPhone.<br />Formatos permitidos: MP4, MOV · Máximo: 200 MB</small></span><span className="button outline dark">Seleccionar video</span><input type="file" accept="video/mp4,video/quicktime,.mp4,.mov" onChange={chooseVideo} /></label>
      {(pendingVideo?.preview || videoUrl) && <div className="admin-video-preview"><video controls playsInline preload="metadata" style={{ aspectRatio: `${videoDimensions.width} / ${videoDimensions.height}`, width: videoDimensions.width > videoDimensions.height ? 'min(100%, 700px)' : 'min(100%, 405px)' }} onLoadedMetadata={(event) => { const video = event.currentTarget; if (video.videoWidth && video.videoHeight) setVideoDimensions({ width: video.videoWidth, height: video.videoHeight }); }} onError={() => setVideoPlaybackWarning(true)} aria-label={`Recorrido en video de ${values.title || 'la propiedad'}`}><source src={pendingVideo?.preview ?? videoUrl} type={pendingVideo ? videoMimeType(pendingVideo.file.name) : videoMimeType(videoStoragePath)} />Tu navegador no puede reproducir este video.</video>{videoPlaybackWarning && videoMimeType(pendingVideo?.file.name ?? videoStoragePath) === 'video/quicktime' && <p className="video-compatibility-note">{pendingVideo ? 'El archivo MOV fue seleccionado correctamente, pero este navegador puede no mostrar la vista previa.' : 'El video fue subido correctamente, pero este navegador puede no reproducir archivos MOV. La reproducción puede variar según el dispositivo.'}</p>}<div><strong>{pendingVideo ? 'Nuevo video pendiente de guardar' : 'Video actual'}</strong>{pendingVideo ? <button type="button" className="text-button" onClick={() => { URL.revokeObjectURL(pendingVideo.preview); setPendingVideo(null); setVideoPlaybackWarning(false); }}>Cancelar reemplazo</button> : initial && videoStoragePath ? <button type="button" className="text-button danger-text" disabled={busy} onClick={() => setConfirmAction('video')}>Eliminar video</button> : null}</div></div>}
    </section>
