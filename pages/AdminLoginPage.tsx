...
export default function AdminLoginPage() {
  ...
  return <main className="admin-login"><section className="admin-login-brand"><a className="logo" href="/"><img src={COMPANY_INFO.logoUrl} alt="Estrella Real Estate" /><span>ESTRELLA REAL ESTATE</span></a><div><span className="eyebrow">Administración segura</span><h1>Gestiona cada propiedad desde un solo lugar.</h1><p>Acceso exclusivo para el equipo autorizado de Estrella Real Estate.</p></div></section><section className="admin-login-form"><form className="admin-auth-card" onSubmit={submit}><span className="eyebrow">Panel administrativo</span><h2>Iniciar sesión</h2>{!isSupabaseConfigured && <div className="form-error">Configura las variables de Supabase antes de iniciar sesión.</div>}{error && <div className="form-error" role="alert">{error}</div>}<div className="input-group"><label htmlFor="admin-email">Email</label><input id="admin-email" type="email" autoComplete="email" required value={email} onChange={(e) => setEmail(e.target.value)} /></div><div className="input-group"><label htmlFor="admin-password">Contraseña</label><input id="admin-password" type="password" autoComplete="current-password" required value={password} onChange={(e) => setPassword(e.target.value)} /></div><button className="button" type="submit" disabled={loading || !isSupabaseConfigured}>{loading ? 'Verificando…' : 'Iniciar sesión'}</button><a className="back-link" href="/">← Volver al sitio web</a></form></section></main>;
}
