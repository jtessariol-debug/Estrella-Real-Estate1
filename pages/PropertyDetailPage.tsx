...
export default function PropertyDetailPage({ slug }: { slug: string }) {
  ...
  return <main className="detail"><div className="container">
    ...
    {property.videoUrl && <section className="property-video"><div><span className="eyebrow">Conoce cada espacio</span><h2>Recorrido en video</h2><p>Explora esta propiedad con mayor detalle antes de coordinar tu visita.</p></div><video controls playsInline preload="metadata" src={property.videoUrl} aria-label={`Recorrido en video de ${property.title}`}>Tu navegador no puede reproducir este video.</video></section>}
