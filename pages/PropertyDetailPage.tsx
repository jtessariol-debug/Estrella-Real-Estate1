...
import MuxPropertyVideo from '../components/MuxPropertyVideo';
import { parseVideoAspectRatio } from '../utils/video';
...
export default function PropertyDetailPage({ slug }: { slug: string }) {
  ...
  const muxVideoReady = property.videoProvider === 'mux' && property.videoStatus === 'ready' && property.muxPlaybackId;
  const hasPropertyVideo = Boolean(muxVideoReady || property.videoUrl);
  const muxAspectRatio = parseVideoAspectRatio(property.videoAspectRatio);
  return <main className="detail"><div className="container">
    <PropertyGallery images={property.images} propertyTitle={property.title} />
    {hasPropertyVideo && <section className="property-video"><div className="property-video-copy"><span className="eyebrow">Conoce cada espacio</span><h2>Recorrido en video</h2><p>Explora esta propiedad con mayor detalle antes de coordinar tu visita.</p></div><div className="property-video-media" style={{ aspectRatio: `${muxVideoReady ? muxAspectRatio.width : videoDimensions.width} / ${muxVideoReady ? muxAspectRatio.height : videoDimensions.height}`, width: (muxVideoReady ? muxAspectRatio.width > muxAspectRatio.height : videoDimensions.width > videoDimensions.height) ? 'min(100%, 720px)' : undefined }}>{muxVideoReady ? <MuxPropertyVideo propertyId={property.id} playbackId={property.muxPlaybackId!} title={property.title} /> : <video controls playsInline preload="metadata" onLoadedMetadata={(event) => { const video = event.currentTarget; if (video.videoWidth && video.videoHeight) setVideoDimensions({ width: video.videoWidth, height: video.videoHeight }); }} aria-label={`Recorrido en video de ${property.title}`}><source src={property.videoUrl} type={videoMimeType(property.videoStoragePath)} />Tu navegador no puede reproducir este video.</video>}</div></section>}
